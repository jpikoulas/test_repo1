...
def locate_anonymize_file(filename, app_dir, err_collection):
  ...
  global_file_path = os.path.join(os.environ['SPLUNK_HOME'], 'etc', anonymize_dir, filename)
  ...
  try:    
    ...
    if app:
      if "/" in app or "\\" in app or ".." in app:
        ...
        app_dir = os.path.join(os.environ['SPLUNK_HOME'], 'etc', 'apps', app)
        if not os.path.isdir(app_dir):
          app_dir = os.path.join(os.environ['SPLUNK_HOME'], 'etc', 'slave-apps', app)
          if not os.path.isdir(app_dir):
