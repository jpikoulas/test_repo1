...
chunked = [true|false]
* EXPERIMENTAL. This option may be dropped or changed in future releases.
* If true, this command supports the experimental "chunked" custom
  search command protocol. If set, the only other commands.conf
  attribute supported is "filename". All other attributes are
  unsupported or negotiated with the search command.
...
maxwait = <integer>
* EXPERIMENTAL. This option may be dropped or changed in future releases.
* If chunked = false, this option has no effect.
* If chunked = true, the value of maxwait is the maximum number of
  seconds the custom search command may pause before producing output.
* If set to 0, the command may pause forever.
* Default is 0

maxchunksize = <integer>
* EXPERIMENTAL. This option may be dropped or changed in future releases.
* If chunked = false, this option has no effect.
* If chunked = true, the value of maxchunksize is maximum size chunk
  (size of metadata plus size of body) the external command may
  produce. If the command tries to produce a larger chunk, the command
  is terminated.
