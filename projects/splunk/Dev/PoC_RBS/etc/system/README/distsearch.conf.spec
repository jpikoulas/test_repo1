...
trySSLFirst = <bool>
* Controls whether the search head attempts HTTPS or HTTP connection when a new
  peer is added, or during a restart. If value is missing true is assumed.
* Defaults to true

peerResolutionThreads = <int>
* Controls how many threads to use to resolve peers during a restart. Or forced
  bundle syncronization.
...
* List of search peers that are members of this group. Comma serparated list
  of host:port in the same format as the servers field of the distributedSearch
  stanza
