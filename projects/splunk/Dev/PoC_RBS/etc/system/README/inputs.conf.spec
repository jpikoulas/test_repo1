...
[tcp:<port>]
* This input stanza is identical to [tcp://<remote server>:<port>], but does not have a remote server restriction.
* See the documentation for [tcp://<remote server>:<port>] for stanza usage instructions.
* [tcp:<port>] has the following supported settings:
connection_host = [ip|dns|none]
queueSize = <integer>[KB|MB|GB]
persistentQueueSize = <integer>[KB|MB|GB|TB]
requireHeader = <bool>
...
* When configured in FIPS mode ssl2 and ssl3 are always disabled regardless of
  this configuration
...
* Check the common name of the client's certificate against this name.
...
* Check the alternate name of the client's certificate against this name.
* If there is no match, assume that Splunk is not authenticated against this
  server.
* requireClientCert must be set to true for this setting to work.
