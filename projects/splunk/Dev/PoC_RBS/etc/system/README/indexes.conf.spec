...
* Controls how often, in seconds, indexer checks status of the child OS
  processes it had jaunched to see if it can launch new processes for queued
  requests.
...
* Script Requirements:
  ...
  * Your script should complete in a reasonable amount of time.
    ...
    * This means that a stalling script for index1 may prevent freezing of
      index2.
...
4.1 and earlier data:
  * To archive: gzip the .tsidx files, as they are highly compressable but
    not recreateable
...
* Important if you have enabled acks on forwarders and have replication
  enabled (via Index Clustering)
* This parameter puts an upper limit on how long events can sit unacked in a
  raw slice
...
* This parameter puts an upper limit on how long an event can sit in raw
  slice.
* If there are any ack''d events sharing this raw slice, this paramater will
...
* Period we force sync tsidx files on streaming targets.  This setting is
 jneeded for multi-site clustering where streaming targets may be primary.
...
* Comma delimted list of Splunk dirs/jars to add to the classpath in the
  Search Head and MR.

vix.env.HUNK_THIRDPARTY_JARS = <list of paths>
* Comma delimted list of 3rd-party dirs/jars to add to the classpath in the
  Search Head and MR.
...
vix.splunk.search.splitter.hive.rowformat.escapted = <escape char>
...
* Can exctract fields with ${field}. I.e: "/data/${server}/...", where server
  will be extracted.
* May start with a schema.
  * The schema of the path specifies which hadoop filesystem implementation to
    use. Examples:
    * hdfs://foo:1234/path, will use a HDFS filesytem implementation
...
# Earliest time extractions - For all 'et' settings, there's an equivalient 'lt' setting.
...
# Latest time exctractions - See "Earliest time exctractions"
...
* Filter for archiving buckets only when they are <seconds> old.
