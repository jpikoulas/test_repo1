...
# Splunk's pdf server.
#
# There is a pdf_server.conf in $SPLUNK_HOME/etc/system/default/. To set
# custom configurations, place a pdf_server.conf in
# $SPLUNK_HOME/etc/system/local/.  For examples, see
# pdf_server.conf.example.  You must restart the pdf server to enable
# configurations.
#
...
* Default setting is '/'.
* For example: if you want to proxy the splunk UI
  at http://splunk:8000/splunkui, then set root_endpoint = /splunkui
...
* The string 'localhost' is a synonym for '127.0.0.1' (or '::1', if your hosts
  file prefers IPv6).
...
* Indicates if all strings that come into CherryPy controller methods are
...
* Once the backlog is reached the server will return an error on receiving
  additional requests.
* Defaults to 10.

max_concurrent = <integer>
* Specifies the maximum number of copies of Firefox that the report server will
  use concurrently to render reports.
...
* If set, the PDF server will only query Splunk app servers on IP addresses
  within the IP networks specified here.
...
* If set, the PDF server will only accept requests from hosts whose IP
  address falls within the IP networks specified here.
...
* If enabled allows screenshots of the X server to be taken for debugging
  purposes.
* Enabling this is a potential security hole as anyone on an IP address
  matching client_ipaddr will be able to see reports in progress.
* Defaults to False.
