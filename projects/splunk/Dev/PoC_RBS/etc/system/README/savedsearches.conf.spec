...
schedule_window = <unsigned int>
* The "window" of time (in minutes) a search may start within for searches that
  do not require a precise scheduled time.  This allows the scheduler greater
  flexibility when prioritizing searches.
* A window must be less that a search's period.
* Windows are not recommended for searches that run every minute.
* Defaults to 0 (meaning "no window").
...
* Comma delimited list of time ranges that each summarized chunk should span.
  ...
  granuality is at the day level should set this to 1d. If you're going to need
...
* Sets the period of time to show the alert in the dashboard. Use
  [number][time-unit] to specify a time.
* For example: 60 = 60 seconds, 1m = 1 minute, 1h = 60 minutes = 1 hour etc
* Defaults to 24h.
...
* auto  - determine whether to track or not based on the tracking setting of
          each action, do not track scheduled searches that always trigger
          actions.
