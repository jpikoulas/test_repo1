...
* When appServerPorts is set to a non-zero value value (the default mode),
  this setting is DEPRECATED.  SSLv2 is now disabled by default in this
  mode.  The exact set of SSL versions allowed is now configurable via the
...
* Specifies whether or not to force the use of FlashChart when rendering
  simple xml into view xml
* Defaults to False
...
* If True, simple xml dashboards and forms will render using the module
  system
* Defaults to False
