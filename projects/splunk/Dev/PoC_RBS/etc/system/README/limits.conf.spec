...
* Provides a limitation to the amount of RAM a batch of events or results will
  use in the memory of search processes.
...
[outputcsv]
protect_dispatch_folders = <bool>
* protect user from overwriting dispatch folders
* Default is true
...
* Absolute value of the largest timeskew in seconds that we will tolerate
  between the native clock on the searchhead and the natic clock on the peer
...
* Maximum time in the past to search for previously triggered alerts. These
  alerts will then be displayed on the Activity -> Triggered Alerts page.
...
* When true, search will issue a warning for a tstats summariesonly=true search if it finds
  a non-hot bucket that has no corresponding datamodel acceleration summary whatsoever.
* Defaults to false
