        ...
        try:
            ...
            if not s.isDisabled():
                ...
                if s.has_key("caCertFile"):
                    self._sslpol._cafile = os.path.expandvars(s["caCertFile"])
                if s.has_key("sslCommonNameList"):
                    self._sslpol._sslCommonNameList = os.path.expandvars(s["sslCommonNameList"])
                if s.has_key("cipherSuite"):
                    self._sslpol._cipherSuite = os.path.expandvars(s["cipherSuite"])
            s = conf["shclustering"]
            if not s.isDisabled():
